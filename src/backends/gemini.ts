...
export class GeminiBackend implements ConsultBackend {
  readonly name = "gemini";

  async available(): Promise<boolean> {
    if (!process.env.GEMINI_API_KEY) return false;
    if (process.env.KIRI_FORCE_GEMINI_CLI_PRESENT === "1") return true;
  ...
  async invoke(prompt: string, cwd: string, timeoutMs: number, model?: string): Promise<SpawnResult> {
    ...
    const cmdOverride = process.env.KIRI_GEMINI_CMD_OVERRIDE;
    const [cmd, ...prefix] = (cmdOverride ?? "gemini").split(" ");
  ...
  parseVerdict(stdout: string): ConsultVerdict | null {
    // Gemini may wrap output in a response envelope
    let text = stdout;
    const envelopeMatch = stdout.match(/"response"\s*:\s*"([\s\S]*?)"/);
    if (envelopeMatch) text = envelopeMatch[1];
